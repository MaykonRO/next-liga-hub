...
export const Route = createFileRoute("/dashboard")({
  head: () => ({
    meta: [
      ...
      {
        name: "description",
        content: "Acompanhe seus campeonatos, partidas e atletas em tempo real.",
      },
    ],
  }),
  component: Dashboard,
              ...
              <div className="flex items-center justify-between p-5 border-b border-border">
                <div>
                  ...
                  <p className="text-xs text-muted-foreground">
                    Visão geral dos torneios em andamento
                  </p>
                </div>
                ...
                <table className="w-full text-sm">
                  ...
                  <tbody>
                    {champs.map((c) => (
                      <tr
                        key={c.name}
                        className="border-t border-border hover:bg-background/30 transition cursor-pointer"
                      >
                        ...
                        <td className="px-3 py-4">
                          <StatusPill variant={c.status as any}>{c.statusLabel}</StatusPill>
                        </td>
...
function Metric({ icon: Icon, label, value, delta, progress, color }: any) {
  ...
  return (
    <div className="p-5 rounded-10xl bg-surface border border-border card-hover">
      <div
        className={`w-10 h-10 rounded-lg border flex items-center justify-center mb-3 ${colorMap[color]}`}
      >
...
const quickActions = [
  ...
  {
    icon: Bell,
    label: "Notificar equipes",
    desc: "Mensagem em massa",
    bg: "bg-success/10",
    color: "text-success",
  },
  {
